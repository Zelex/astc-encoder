...
int astcenc_main(
	...
	if (operation & ASTCENC_STAGE_COMPRESS)
	{
		...
		if (cli_config.lz_optimize)
		{
			...
			astcenc_optimize_for_lz(buffer, buffer_exhaustive, buffer_size, blocks_x, blocks_y, blocks_z, config.block_x, config.block_y, config.block_z, data_type, channel_weights, cli_config.thread_count, cli_config.silentmode, cli_config.lz_optimize_rdo, cli_config.lz_optimize_effort);
		}
